# 📊 LEVEL UP

## 📋 Pré-requisitos

### 📦 Bibliotecas Python

``` bash
pip install oracledb
pip install requests
pip install pandas
```

------------------------------------------------------------------------

## 🔗 Links Úteis

<<<<<<< HEAD
-   [Vídeo de Python](https://www.youtube.com/watch?v=KwZ7Ok4hV8M)
=======
-   [Vídeo de Python](https://youtu.be/KwZ7Ok4hV8M)\
>>>>>>> 004c0203ee438c9e918e2a43474f10adfe93651b
-   [Vídeo Pitch do
    Projeto](https://www.youtube.com/shorts/_mLTC6II3CQ)
-   [Repositório no
    GitHub](https://github.com/victoralves10/levelup-logica-python)

------------------------------------------------------------------------

## 👥 Integrantes do Projeto

| Nome                    | RM     | Contato                                             |
| ----------------------- | ------ | --------------------------------------------------- |
| Eduardo Batista Locaspi | 561713 | [rm561713@fiap.com.br](mailto:rm561713@fiap.com.br) |
| Victor Alves Lopes      | 561833 | [rm561833@fiap.com.br](mailto:rm561833@fiap.com.br) |


------------------------------------------------------------------------

## 🚀 Sobre o Projeto

O **LEVEL UP** é um sistema completo de cadastro e gestão de empresas
desenvolvido em Python, com integração ao banco de dados Oracle.
O sistema permite o CRUD completo (**Create, Read, Update, Delete**) de
empresas, incluindo gestão de endereços e credenciais de login.

------------------------------------------------------------------------

## ✨ Funcionalidades Principais

### 🏢 Cadastro de Empresas

-   Cadastro completo com dados da empresa, endereço e login
-   Validação automática de CNPJ, e-mail e CEP
-   Integração com ViaCEP para preenchimento automático de endereços
-   Confirmação em múltiplas etapas antes do cadastro

### 🔍 Consultas Avançadas

-   Pesquisa genérica: busca em todos os campos das tabelas
-   Pesquisa por ID: consulta específica por identificador
-   Pesquisa geral: lista completa de todas as empresas
-   Exportação para JSON: salva resultados em arquivo

### ✏️ Atualização de Dados

-   Edição campo a campo com preview dos dados atuais
-   Atualização em cascata: modifica empresa, login e endereço
-   Validação em tempo real durante a edição

### 🗑️ Exclusão Segura

-   Confirmação dupla antes da exclusão
-   Exclusão em cascata: remove empresa, login e endereço
-   Preview dos dados que serão excluídos

------------------------------------------------------------------------

## 🛠️ Tecnologias Utilizadas

-   **Python 3.x** --- Linguagem principal
-   **Oracle Database** --- Banco de dados
-   **Pandas** --- Manipulação de dados
-   **Requests** --- Integração com API ViaCEP
-   **JSON** --- Exportação de dados

------------------------------------------------------------------------
